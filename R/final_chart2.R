...
#' of parameters will be generated. The coefficients for linear and square terms are
#' in [0,1] while those for corss products are in [-1,0].
...
final2 <- function(n=20, expen=50, times=1000, cost=0) {
    ...
    for (kind in c("steep","median","least")) {
        ...
        for (connection in 1:5) {
            Single[connection] <- repeat.climb2(n, expen, times, connection, kind, cost)
        }
